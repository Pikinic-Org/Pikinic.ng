...
The homepage sets the standard, and every other page should reuse its patterns rather than invent new ones.
...
- **Cards:** white (`surface-primary`) on the off-white page, `rounded-2xl`, no borders and no shadows. The one highlighted card in a set uses `green-900` with the brand pattern.
